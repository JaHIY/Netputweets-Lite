...
class Twitter_Autolink {
	...
	public function autoLinkHashtags($tweet) {
		...
		return preg_replace('$(^|[^0-9A-Z&/]+)([#＃]+)([0-9A-Z_]*[A-Z_]+[a-z0-9_üÀ-ÖØ-öø-ÿ]*)$iu',
			'${1}<a href="' . $this->get_base() . 'hash/' . '${3}" title="#${3}" class="' . $this->urlClass . ' ' . $this->hashtagClass . '" rel="tag">${2}${3}</a>',
							$tweet);
	}
	...
	private function replacementURLs($matches) {
		...
		if (substr($matches[3], 0, 7) == 'http://' || substr($matches[3], 0, 8) == 'https://') {
			$replacement .= '<a href="' . $matches[3] . '" rel="external noreferrer">' . $matches[3] . '</a>';
		} else {
			$replacement .= '<a href="http://' . $matches[3] . '" rel="external noreferrer">' . $matches[3] . '</a>';
		}
		return $replacement;
	}
